...
const AnalyticsSection = ({ allFormulas }) => {
  ...
  const barData = [...allFormulas]
    .sort((a, b) => b.costPerMT - a.costPerMT).slice(0, 6)
    .map((f) => ({
      name: f.formulaName.length > 10 ? f.formulaName.slice(0, 10) + "…" : f.formulaName,
      cost: f.costPerMT,
    }));

  const catMap = {};
  allFormulas.forEach((f) => {
    const name = f.category?.categoryName || "Uncategorized";
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
      <div className={`lg:col-span-2 ${cardCls}`}>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-gray-800 dark:text-white">Top Formulas by Cost / MT</h3>
        </div>
        {barData.length > 0 ? (
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={barData} barSize={28}>
              ...
              <Tooltip contentStyle={tip} formatter={(v) => [`Rs ${v.toLocaleString()}`, "Cost/MT"]} />
      ...
      <div className={cardCls}>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-gray-800 dark:text-white">By Category</h3>
        </div>
        {pieData.length > 0 ? (
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
